...
void CSubLine::Draw(int x, int w, int align)
{
	...
	{
		surface()->DrawSetTextPos(x + ((w - m_TextLength) >> 1), m_YPos);
	}
	else if(nTextAlign == ALIGN_RIGHT)
	{
		surface()->DrawSetTextPos(x + (w - m_TextLength), m_YPos);
	}
	else
	{
		surface()->DrawSetTextPos(x, m_YPos);
	}
	surface()->DrawPrintText(m_Sentence, m_Length);
...
CSubLine *SubtitlePanel::AddLine(CDictionary *dict, wchar_t *sentence, int len, float starttime, float duration, int textlength)
{
	CSubLine *Line = new CSubLine(this, dict);
	m_BackLines[m_BackLines.AddToTail()] = Line;

	Line->m_Sentence = sentence;
	Line->m_Length = len;
	Line->m_TextLength = textlength;
	Line->m_StartTime = starttime;
...
void SubtitlePanel::StartSubtitle(CDictionary *dict, float starttime)
{
	...
	wchar_t szBuf[1024];
	wchar_t *pStart = dict->m_szSentence;
	wchar_t *p = pStart;
	...
	while(*pStart)
	{
		...
		AddLine(dict, pStart, nCharNum, max(flLatestStart, starttime + flPercentStart * flDuration), flDuration * flPercentDuration, nWide);
