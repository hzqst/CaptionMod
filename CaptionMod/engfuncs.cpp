...
void Engine_FillAddress(void)
{
	...
	{
		gCapFuncs.S_StartDynamicSound = (void (*)(int, int, sfx_t *, float *, float, float, int, int))Search_Pattern(S_STARTDYNAMICSOUND_SIG_NEW);
		...
		gCapFuncs.S_LoadSound = (sfxcache_t *(*)(sfx_t *, channel_t *))Search_Pattern_From(S_StartDynamicSound, S_LOADSOUND_SIG_NEW);
		Sig_FuncNotFound(S_LoadSound);
	}
	else
	{
		gCapFuncs.S_StartDynamicSound = (void (*)(int, int, sfx_t *, float *, float, float, int, int))Search_Pattern(S_STARTDYNAMICSOUND_SIG);
		Sig_FuncNotFound(S_StartDynamicSound);
		...
		gCapFuncs.S_LoadSound = (sfxcache_t *(*)(sfx_t *, channel_t *))Search_Pattern_From(S_StartDynamicSound, S_LOADSOUND_SIG);
		Sig_FuncNotFound(S_LoadSound);
	}
}
